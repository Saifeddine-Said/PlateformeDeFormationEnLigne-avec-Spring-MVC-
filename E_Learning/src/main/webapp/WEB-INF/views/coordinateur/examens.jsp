...
<link rel="stylesheet" href="<%=request.getContextPath()%>/resources/plugins/steps/css/jquery-steps.css">
<!--/ Plugins stylesheet : optional -->
...
<script type="text/javascript">
	...
	function typeExamFunction() {
		var index = $("#typeSelect")[0].selectedIndex;
		if (index > 0) {
			$("#formationBlock").show();
		} else {
			$("#formationBlock").hide();
	...
	function loadModuleParFormation() {
		...
		if (index > 0) {
			$.getJSON("listModuleParFormation", {
				...
				if (data.length > 0) {
					$("#moduleBlock").show();
					$("#modulesSelect option").remove();
				}
				var html = "<option value=''>Selectionner un module</option>";
				for (var index = 0; index < data.length; index++) {
					html += "<option value='"+data[index].idModule+"'>"
							+ data[index].nomModule + "</option>";
				}
				$("#modulesSelect").html(html);
			});
		} else {
			$("#moduleBlock").hide();
		}
	...
	function loadChapitresParModule() {
		var index = $("#modulesSelect")[0].selectedIndex;
		if (index > 0 && $("#typeSelect")[0].selectedIndex == 1) {
			$("#chapitreBlock").show();
			$.getJSON(
							"listChapitreParModule",
							{
								idModule : $("#modulesSelect").val(),
								ajax : "true"
							},
							function(data) {
								if (data.length > 0) {
								...
								var html = "<option value='null'>Selectionner un chapitre</option>";
								for (var index = 0; index < data.length; index++) {
									html += "<option value='"+data[index].idChapitre+"'>"
											+ data[index].titreChapitre
											+ "</option>";
								}
								$("#chapitreSelect").prop("required", true);
								$("#chapitreSelect").html(html);
							});
		} else {
			$("#chapitreSelect").prop("required", false);
			$("#chapitreBlock").hide();
		}
	}
	...
	function submitEtape1() {
		$.ajax({
		    data:$("#formEtape1").serialize() + "&numEtape=1",
		    url : "etapes",
		    type : "POST",
		    success : function(response) {
		    	$("#examTab1").attr('class', '');
		    	$("#tabone2").attr('class', 'tab-pane');
		    	
		    	
		    	$("#examTab2").attr('class', ' active');
		    	$("#tabtwo2").attr('class', ' active');
		    }
		});
		return false;
	}
	
	function submitEtape2() {
	    $.ajax({
	    	data:$("#formEtape2").serialize() + "&numEtape=2",
		    url : "etapes",
	        type : "POST",
	        success : function(response) {
	          $("#examTab2").attr('class', '');
	          $("#tabtwo2").attr('class', 'tab-pane');
	          $("#tabtwo1").attr('class', 'tab-pane');
	          
	          $("#examTab3").attr('class', ' active');
	          $("#tabthree2").attr('class', ' active');
	        }
	    });
	    return false;
	  }
	...
	function saveExam() {
			$.ajax({
			data:$("#formEtape3").serialize() + "&numEtape=3",
		    url : "etapes",
	        type : "POST",
	        success : function(response) {
	        }
	...
	function resetFormQuestion() {
	      ...
	      var html = "<div class='form-group'>" 
	    	      +"<input type='hidden' name='nbrReps' value='2' id='nbrReps' />"
	            +"<label class='control-label col-sm-4'>Réponse (1)</label>"
	            +"<div class='col-sm-8'>"
	            +"<div class='row'>"
	            +"<div class='col-md-4 pr5'>"
	            +"<input name='reponse1' type='text' class='form-control' placeholder='Réponse' />"
	            +"</div>"
	            +"<div class='col-md-4 pr5'>"
	            +"<input name='feedBack1' type='text' class='form-control' placeholder='Feedback' />"
	            +"</div>"
	            +"<div class='col-md-2 pr5'>"
	            +"<input name='poids1' type='text' class='form-control' placeholder='Poids' />"
	            +"</div>"
	            +"<div class='col-md-2 pr5 checkbox custom-checkbox'>"
	            +"<input type='checkbox' name='reponseCorrecte1' id='reponseCorrecte1' /> <label for='reponseCorrecte1'>&nbsp;&nbsp;Réponse correcte</label>"
	            +"</div>"
	            +"</div>"
	            +"</div></div>"+"<div class='form-group'>" 
	            +"<label class='control-label col-sm-4'>Réponse (2)</label>"
	            +"<div class='col-sm-8'>"
	            +"<div class='row'>"
	            +"<div class='col-md-4 pr5'>"
	            +"<input name='reponse2' type='text' class='form-control' placeholder='Réponse' />"
	            +"</div>"
	            +"<div class='col-md-4 pr5'>"
	            +"<input name='feedBack2' type='text' class='form-control' placeholder='Feedback' />"
	            +"</div>"
	            +"<div class='col-md-2 pr5'>"
	            +"<input name='poids2' type='text' class='form-control' placeholder='Poids' />"
	            +"</div>"
	            +"<div class='col-md-2 pr5 checkbox custom-checkbox'>"
	            +"<input type='checkbox' name='reponseCorrecte2' id='reponseCorrecte2' /> <label for='reponseCorrecte2'>&nbsp;&nbsp;Réponse correcte</label>"
	            +"</div>"
	            +"</div>"
	            +"</div></div>";
	      $("#reponseBlock").html(html);
	      $("#nbrReps").val("2");
	      $("#questNbr").text(nbr);
	      $("#nbrQuest").val(nbr);
	...
	function ajouterReponse() {
		...
		html += "<div class='form-group'>" 
			+"<label class='control-label col-sm-4'>Réponse ("+ nbr +")</label>"
			+"<div class='col-sm-8'>"
			+"<div class='row'>"
		+"<div class='col-md-4 pr5'>"
			+"<input name='reponse"+nbr+"' type='text' class='form-control' placeholder='Réponse' />"
				+"</div>"
			+"<div class='col-md-4 pr5'>"
			+"<input name='feedBack"+nbr+"' type='text' class='form-control' placeholder='Feedback' />"
				+"</div>"
			+"<div class='col-md-2 pr5'>"
			+"<input name='poids"+nbr+"' type='text' class='form-control' placeholder='Poids' />"
				+"</div>"
			+"<div class='col-md-2 pr5 checkbox custom-checkbox'>"
			+"<input type='checkbox' name='reponseCorrecte"+nbr+"' id='reponseCorrecte"+nbr+"' /> <label for='reponseCorrecte"+nbr+"'>&nbsp;&nbsp;Réponse correcte</label>"
				+"</div>"
			+"</div>"
	+"</div></div>";
	
		$("#reponseBlock").append(html);
		$("#nbrReps").val(nbr);
		return false;
	}
...
<body>
	...
	<aside class="sidebar sidebar-left sidebar-menu">
		...
		<section class="content slimscroll">
			...
			<ul class="topmenu topmenu-responsive" data-toggle="menu">
				...
				<li class="active open"><a href="${examenLink }" data-toggle="submenu" data-target="#page" data-parent=".topmenu"> <span class="figure"><i class="ico-file6"></i></span> <span class="text">Examens</span> <span class="arrow"></span>
				</a></li>
		...
		<div class="container-fluid">
			...
			<div class="page-header page-header-block">
				<div class="page-header-section">
					<h4 class="title semibold">Examens</h4>
				</div>
				<div class="page-header-section">
					<!-- Toolbar -->
					<div class="toolbar">
			...
			<div class="">
				...
				<div class="section-header mb15">
					<h5 class="semibold">Nouveau examen</h5>
				</div>
				<!--/ section header -->
				<!-- tab -->
				...
				<ul class="nav nav-tabs nav-justified">
					<li class="${tab1 }" id="examTab1"><a href="#tabone2" data-toggle="tab" aria-expanded="false">Type d'examen</a></li>
					<li class="${tab2 }" id="examTab2"><a href="#tabtwo2" data-toggle="tab" aria-expanded="true">Détails d'examen</a></li>
					<li class="${tab3 }" id="examTab3"><a href="#tabthree2" data-toggle="tab" aria-expanded="false">Questions</a></li>
				</ul>
				<!--/ tab -->
				<!-- tab content -->
				<div class="tab-content panel">
					<div class="tab-pane  ${tab1 }" id="tabone2">
						<f:form modelAttribute="examen" class="form-horizontal form-striped panel panel-default" onsubmit="return submitEtape1();" id="formEtape1" action="etape1">
							<c:if test="${typeExamen == null }">
								<div class="panel-body">
									<div class="form-group">
										<label class="control-label col-sm-4">Type d'examen</label>
										<div class="col-sm-8">
											<f:select path="typeExamen" name="type" class="form-control" onclick="typeExamFunction();" id="typeSelect">
												<f:option value="">selectionnez le type d'examen</f:option>
												<f:option value="EE">Examen d'evaluation</f:option>
												<f:option value="EF">Examen final</f:option>
											</f:select>
										</div>
									</div>
									<div class="form-group" id="formationBlock" style="display: none;">
										<label class="control-label col-sm-4">Formation</label>
										<div class="col-sm-8">
											<f:select path="formation.idFormation" items="${listFormations }" itemLabel="nomFormation" itemValue="idFormation" class="form-control" onchange="loadModuleParFormation();" id="formationSelect" required="required" />
										</div>
									</div>
									<div class="form-group" id="moduleBlock">
										<label class="control-label col-sm-4">Module</label>
										<div class="col-sm-8">
											<f:select path="module.idModule" items="${listModules }" itemLabel="nomModule" itemValue="idModule" class="form-control" onchange="loadChapitresParModule();" id="modulesSelect" required="required" />
										</div>
									</div>
									<div class="form-group" id="chapitreBlock">
										<label class="control-label col-sm-4">Chapitre</label>
										<div class="col-sm-8">
											<f:select path="chapitres.idChapitre" items="${listChapitres }" itemLabel="titreChapitre" itemValue="idChapitre" class="form-control" id="chapitreSelect" />
										</div>
									</div>
								</div>
							</c:if>
							<div class="panel-footer">
								<button type="submit" class="btn btn-primary">Enregistrer et passer a l'etape suivante</button>
							</div>
						</f:form>
					</div>
					<div class="tab-pane ${tab2 }" id="tabtwo2">
						<f:form modelAttribute="examen" class="form-horizontal form-striped panel panel-default" action="etape2" id="formEtape2" onsubmit="return submitEtape2();">
							<div class="panel-body">
								<div class="form-group">
									<label class="control-label col-sm-4">Titre</label>
									<div class="col-sm-8">
										<f:input path="examen.titreExamen" name="street" type="text" class="form-control" placeholder="Titre de l'examen" />
									</div>
								</div>
								<div class="form-group">
									<label class="control-label col-sm-4">Description</label>
									<div class="col-sm-8">
										<f:input path="examen.descriptionExamen" name="street" type="text" class="form-control" placeholder="Description de l'examen" />
									</div>
								</div>
								<div class="form-group">
									<label class="control-label col-sm-4">Score minimum</label>
									<div class="col-sm-8">
										<f:input path="examen.scoreMini" name="street" type="text" class="form-control" placeholder="Score minimum pour reussir l'examen" />
									</div>
								</div>
							</div>
							<div class="panel-footer">
					...
					<div class="tab-pane ${tab3 }" id="tabthree2">
						<f:form modelAttribute="examen" class="form-horizontal form-striped panel panel-default" id="formEtape3" onsubmit="return saveExam();">
							<div class="panel-body">
								<div class="form-group">
									<input type="hidden" name="nbrQuest" value="1" id="nbrQuest" /> <label class="control-label col-sm-4">Question (<span id="questNbr">1</span>)
									</label>
									<div class="col-sm-8">
										<f:input path="questions.titreQuestion" id="titreQuestion" type="text" class="form-control" placeholder="Question" />
									</div>
								</div>
								<div class="form-group">
									<label class="control-label col-sm-4">Type de reponse</label>
									<div class="col-sm-8">
										<f:select path="typeQuestion" name="size" class="form-control">
											<f:option value="CM">Choix multiple</f:option>
											<f:option value="CU">Choix unique</f:option>
										</f:select>
									</div>
								</div>
								<div id="reponseBlock">
									<div class="form-group">
										<input type="hidden" name="nbrReps" value="2" id="nbrReps" /> <label class="control-label col-sm-4">Réponse (1)</label>
										<div class="col-sm-8">
											<div class="row">
												<div class="col-md-4 pr5">
													<input name="reponse1" type="text" class="form-control" placeholder="Réponse" />
								...
								<div class="form-group">
									<div class="col-sm-8">
										<div class="row">
											<div class="control-label col-sm-6 pr5">
												<button class="btn btn-primary" title="Nouvelle reponse" onclick="return ajouterReponse();">
													<span class="ico-plus-circle2"></span> &nbsp;Nouvelle reponse
												</button>
											</div>
										</div>
									</div>
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/resources/plugins/steps/js/jquery-steps.js"></script>
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/resources/javascript/backend/forms/wizard.js"></script>
	<!--/ Plugins and page level script : optional -->
