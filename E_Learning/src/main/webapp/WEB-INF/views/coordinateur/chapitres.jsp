...
function loadModuleParFormation() {
	...
	if (index > 0) {
		$.getJSON("listModuleParFormation", {
			idCoordinateur : $("#idCoordinateur").val(),
			ajax : "true"
		}, function(data) {
			if (data.length > 0) {
				$("#moduleBlock").animate( { "opacity": "show"} , 1000 );
		...
		$("#moduleBlock").animate( { "opacity": "hide"} , 1000 );
		$("#confirmBlock").animate( { "opacity": "hide"} , 1000 );
	}
}

function selectChapitres() {
var index = $("#modulesSelect")[0].selectedIndex;
if(index > 0) {
	$("#confirmBlock").animate( { "opacity": "show"} , 1000 );
	$.getJSON(
...
function afficherChpitresParModule(){
	$.getJSON(
			"listChapitreParModule",
			{
				idModule : $("#modulesSelect").val(),
			...
			function(data) {
				for (var index = 0; index < data.length; index++) {
					$("#nomChapitre").text(data[index].titreChapitre);					
				}
			});
	return false;
}
...
<body>
	...
	<aside class="sidebar sidebar-left sidebar-menu">
		...
		<section class="content slimscroll">
			...
			<ul class="topmenu topmenu-responsive" data-toggle="menu">
				...
				<li class="active open"><a href="${chapitreLink }" data-toggle="submenu" data-target="#table" data-parent=".topmenu"> <span class="figure"><i class="ico-numbered-list"></i></span> <span class="text">Chapitres</span> <span class="arrow"></span>
				</a></li>
	...
	<section id="main" role="main">
				...
				<div class="page-header-section">
					...
					<div class="toolbar">
						<ol class="breadcrumb breadcrumb-transparent nm">
							<li><button type="submit" class="btn btn-primary" data-toggle="collapse" data-target="#ajouterChapitre">
									<span class="ico-plus-circle2"></span> &nbsp;Nouveau chapitre
								</button></li>
						</ol>
					</div>
					<!--/ Toolbar -->
     ...
     <div class="panel panel-default ${collapse }" data-expanded="false" id="ajouterChapitre">
        <!-- Form horizontal layout -->
        <f:form modelAttribute="chapitre" method="post" class="form-horizontal panel panel-default" action="enregistrerChapitre">
          <div class="panel-heading">
            <h3 class="panel-title">Nouveau chapitre</h3>
          </div>
          <div class="panel-body">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-8">
                <f:select path="module.idModule" items="${modules }" itemLabel="nomModule" itemValue="idModule" class="form-control" />
              </div>
            </div>
          </div>
          <div class="panel-footer">
            <button type="submit" class="btn btn-primary">Enregistrer</button>
            <button type="reset" class="btn btn-danger" data-toggle="collapse" data-target="#ajouterChapitre">Annuler</button>
          </div>
        </f:form>
        <!--/ Form horizontal layout -->
      </div>
